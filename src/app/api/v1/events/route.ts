  ...
  try {
    ...
    if (!authHeader.startsWith("Bearer")) {
      return NextResponse.json(
        {
          message:
            "Invalid auth header format Expected: 'Bearer <YOUR_API_KEY>'",
        },
        { status: 401 }
      )
    }
    ...
    if (!apiKey || apiKey.trim() === "") {
      return NextResponse.json({ message: "Invalid API_KEY" }, { status: 401 })
    }
    ...
    if (!category) {
      return NextResponse.json(
        {
          message: `you don't have a category names "${validationResult.category}"`,
        },
        { status: 404 }
      )
    }
